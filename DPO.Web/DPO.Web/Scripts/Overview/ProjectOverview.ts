...
module Overview {
    export class ProjectOverview {
        private static MAXWIDGETS: number = 13
        private search: IWidgetSearch;

        constructor(search: IWidgetSearch) {
            this.search = search;
            ...
            $content.on('click', '#addNewTile', function () {
                ...
                if ($('.project-overview-inner li').length >= ProjectOverview.MAXWIDGETS) $('#addNewTile').hide();
            });
        ...
        createWidget(storedData: any, insertBeforeElement?: JQuery) {
            var templateId: string = storedData.templateId;
